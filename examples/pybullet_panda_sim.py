...
class PandaSim():
    def __init__(self, bullet_client, base_pos, base_rot):
        ...
        self.panda = self.bullet_client.loadURDF(os.path.join(CUR_PATH,"robo_urdf/panda/panda.urdf"), self.base_pos,
...
def test_panda_sim():
    ...
    p.setAdditionalSearchPath(pybullet_data.getDataPath())  # for plane.urdf and textures
    p.setGravity(0, 0, -9.81)

    # Load plane
    p.loadURDF("plane.urdf")

    # Panda base position and orientation
    base_pos = [0, 0, 0]
    ...
    spheres = SphereManager(p)
    obstacle_array = np.array([
        [0.5, 0, 0.5, 0.05],  # x, y, z, radius
        [0.3, 0.2, 0.4, 0.03]
    ])
    spheres.initialize_spheres(obstacle_array)

    # Main loop
    t = 0
    while True:
        t += 0.01
        joint_positions = [0.3 * math.sin(t + i) for i in range(7)]
